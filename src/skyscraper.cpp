...
Skyscraper::Skyscraper(Vector3f pos, float width, float height, float depth, int roofs): Building(pos)
{
...
void Skyscraper::init()
{
	...
	for (int i = 1; i <= roofs; i++)
	{
		...
		Vector3f parentPos = getPosition();
		Vector3f nPos = { (width - nWidth) / 2, 0, (depth - nDepth) / 2 };
